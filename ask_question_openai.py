#!/usr/bin/env python3
"""
FAQボット - 単発質問スクリプト (OpenAI Embeddings版)
コマンドライン引数で質問を受け取り、回答を生成します。
OpenAIの高精度な埋め込みモデルを使用します。

使い方:
  python ask_question_openai.py "質問内容"
"""
...
def load_vectordb():
    ...
    if not os.path.exists(CHROMA_DB_DIR):
        ...
        print("先に prepare_database_openai.py を実行してデータベースを作成してください。")
        return None
...
def main():
    ...
    # コマンドライン引数のチェック
    if len(sys.argv) < 2:
        print("使い方: python ask_question_openai.py \"質問内容\"")
        print("\n例:")
        print('  python ask_question_openai.py "景品表示法とは何ですか？"')
        print('  python ask_question_openai.py "懸賞の景品の上限金額はいくらですか？"')
        sys.exit(1)
    ...
    print("=" * 60)
    print("FAQボット (OpenAI Embeddings版)")
    print("=" * 60)
    print(f"\n質問: {query}\n")
    ...
    print("関連情報を検索中（OpenAI Embeddings使用）...")
